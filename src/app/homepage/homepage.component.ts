...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, CommonModule, KeyboardComponent, AlertComponent],
...
export class HomepageComponent implements OnInit {
  ...
  */
  showCustomAlert(message: string, type: 'success' | 'error' = 'success') {
  ...
  */
  checkCurrentRow() {
    ...
    if (!isComplete) {
      this.showCustomAlert('Please fill in all letters!', 'error');
      return;
    }
    ...
    if (!this.isValidWord(guess)) {
      this.showCustomAlert('Not a valid 5-letter word!', 'error');
      return;
    }
    ...
    if (guess === secretWord) {
      setTimeout(() => {
        this.showCustomAlert('Congratulations! You found the word!', 'success');
      }, 100);
    ...
    if (row < 5) {
      ...
      setTimeout(() => {
        this.showCustomAlert(`Game Over! The word was: ${secretWord.toUpperCase()}`, 'error');
        this.redobutton = true;
      }, 100);
    }
  }
